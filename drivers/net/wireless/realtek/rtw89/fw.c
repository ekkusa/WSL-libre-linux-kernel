				 ...
				 int *used_fw_format)
{
	...
	for (fw_format = chip->fw_format_max; fw_format >= 0; fw_format--) {
		...
		ret = reject_firmware(&firmware, fw_name, device);
		if (!ret) {
				   ...
				   const char *fw_name, bool nowarn)
{
	...
	if (nowarn)
		ret = firmware_reject_nowarn(&req->firmware, fw_name, rtwdev->dev);
	else
		ret = reject_firmware(&req->firmware, fw_name, rtwdev->dev);
